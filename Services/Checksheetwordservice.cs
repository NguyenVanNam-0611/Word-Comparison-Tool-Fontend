...
namespace Diff_tool.Services
{
    ...
    "\r\n"
        ...
        private void SetCellContentAsNestedTable(TableCell outerCell, TableRowChangeDto subRow, string side)
        {
            ...
            bool isRowDeleted =
                side == "left" &&
                (subRow.Type == "row_deleted" || subRow.Type == "table_row_deleted");

            bool isRowInserted =
                side == "right" &&
                (subRow.Type == "row_added" || subRow.Type == "row_inserted" || subRow.Type == "table_row_added");
            ...
            else
            {
                foreach (var cell in cells.Where(c => c != null))
                {
                    ...
                    {
                        nestedTc = new TableCell();
                        nestedTc.AppendChild(new TableCellProperties(
                    ...
                    else
                    {
                        ...
                        MakeTextRun(
                            ...
                            deleted: isRowDeleted,
                            inserted: isRowInserted
                        )
                    ));
                    }
            ...
            foreach (var block in blocks)
            {
                ...
                else if (block.Type == "image" && block.Payload?.ImageUrl != null)
                {
                    ...
                    AppendImage(
                        ...
                        widthEmu: block.Payload.WidthEmu,
                        heightEmu: block.Payload.HeightEmu,
                        widthPx: block.Payload.WidthPx,
                        heightPx: block.Payload.HeightPx,
                        deleted: isDeleted,
                        inserted: isInserted
                    );
                    tc.AppendChild(para);
    ...
    TableCell outerCell, List<CellDto> cells, bool isDeleted, bool isInserted)
        ...
        private void FillDataRow(TableRow row, string heading, ChangeDto change, TableRowChangeDto? subRow)
        {
            ...
            SetCellContent(cells[0], new[] { new RunInfo { Text = GetPageText(change, subRow) } }, center: true);
        ...
        private static string GetPageText(ChangeDto change, TableRowChangeDto? subRow = null)
        {
            bool isTableRowDeleted =
                subRow?.Type == "row_deleted" ||
                subRow?.Type == "table_row_deleted";

            int? start =
                isTableRowDeleted
                    ? (
                        GetPageStart(change.Left)
                        ?? change.Left?.Page
                        ?? GetPageFromNode(change.Left?.Node)
                        ?? GetPageStart(change.Right)
                        ?? change.Right?.Page
            ...
            int? end =
                isTableRowDeleted
                    ? (
                        GetPageEnd(change.Left)
                        ?? start
                    )
                    : (
                        GetPageEnd(change.Right)
                        ?? GetPageEnd(change.Left)
                        ?? start
                    );
        ...
        private static int? GetPageStart(BlockSideDto? side)
        {
            return side?.PageStart
                ?? GetIntFromNodeContent(side?.Node, "page_start");
        }

        private static int? GetPageEnd(BlockSideDto? side)
        {
            return side?.PageEnd
                ?? GetIntFromNodeContent(side?.Node, "page_end");
        }

        private static int? GetIntFromNodeContent(NodeDto? node, string key)
        {
            var content = node?.Content;
            if (content == null) return null;

            if (!content.TryGetValue(key, out var el))
                return null;
        ...
        private static int? GetIntFromSideOrNode(BlockSideDto? side, string key)
        {
            if (side == null)
                return null;

            var content = side.Node?.Content;
            if (content != null && content.TryGetValue(key, out var el))
            {
                if (el.ValueKind == JsonValueKind.Number && el.TryGetInt32(out var n))
                    return n;
    ...
    string direction = "left")
            ...
            int? heightPx = null)
        {
            ...
            if (widthEmu.HasValue && heightEmu.HasValue && widthEmu > 0 && heightEmu > 0)
            {
                wEmu = widthEmu.Value;
                hEmu = heightEmu.Value;
            }
            else if (widthPx.HasValue && heightPx.HasValue && widthPx > 0 && heightPx > 0)
            {
                wEmu = widthPx.Value * 9525L;
                hEmu = heightPx.Value * 9525L;
            }
            else
            {
                // fallback nhỏ, tránh ảnh bung khổng lồ
                wEmu = 180 * 9525L;
                hEmu = 120 * 9525L;
            }

            // Clamp ảnh để không phá layout checksheet
            const long maxWidthEmu = 220 * 9525L;

            if (wEmu > maxWidthEmu)
            {
                double ratio = (double)maxWidthEmu / wEmu;
                wEmu = maxWidthEmu;
                hEmu = Math.Max(1, (long)(hEmu * ratio));
            }
    ...
    public class ContentBlockPayloadDto
    {
        ...
        [JsonPropertyName("width_emu")] public long? WidthEmu { get; set; }
        [JsonPropertyName("height_emu")] public long? HeightEmu { get; set; }
